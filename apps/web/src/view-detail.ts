...
import { escapeHtml, formatEastern } from './html';

const STUDY_PRIOR = { winRate: '58.3%', median: '+1.83%' } as const;

const refs = (inputRefs: readonly InputReference[]) =>
	`<ul>${inputRefs.map((ref) => `<li>${escapeHtml(ref.kind)} ${escapeHtml(ref.id)} · ${escapeHtml(ref.role)}</li>`).join('')}</ul>`;

const factList = (rows: CockpitCandidateDetail['qualifyingFacts']) => {
	if (!rows.length) return '<p>No causing filings on this row</p>';
	return `<ul>${rows.map((row) => `<li>${escapeHtml(row.reportingOwnerName)} ${escapeHtml(row.transactionCode)} ${escapeHtml(row.shares)} @ ${escapeHtml(row.pricePerShare)}</li>`).join('')}</ul>`;
};
...
export const eligibilityPage = (detail: CockpitEligibilityDetail) => {
	...
	return `<main><h1>Eligibility Decision</h1><p>${escapeHtml(detail.issuerName)} ${escapeHtml(detail.issuerCik)} · ${detail.eligible ? 'eligible' : 'ineligible'}</p><h2>Failed checks</h2>${checks}${link(`/candidates/${detail.candidateSnapshotId}`, 'Candidate Snapshot')}${link(detail.packetId ? `/packets/${detail.packetId}` : undefined, 'Decision Packet')}${link(detail.signalId ? `/signals/${detail.signalId}` : undefined, 'Signal')}<h2>Input references</h2>${refs(detail.inputRefs)}</main>`;
};

export const packetPage = (detail: CockpitPacketDetail) =>
	`<main><h1>Decision Packet</h1><p>${escapeHtml(detail.issuerName)} ${escapeHtml(detail.issuerCik)}</p><p>${escapeHtml(detail.strategyVersion)} · ${escapeHtml(detail.policyVersion)} · calendar ${escapeHtml(detail.calendarVersion)}</p><p>window ${escapeHtml(formatEastern(detail.decisionWindowOpen))}</p>${link(`/eligibility/${detail.eligibilityDecisionId}`, 'Eligibility Decision')}${link(detail.signalId ? `/signals/${detail.signalId}` : undefined, 'Signal')}<h2>Input references</h2>${refs(detail.inputRefs)}</main>`;
...
export const funnelPage = (population: CockpitFunnelPopulation) => {
	const rows = population.rows.length
		? population.rows
				...
				.join('')
		: '<li>No rows in this population</li>';
	return `<main><h1>Funnel · ${escapeHtml(population.stage)}</h1><p>${population.count} in stored population${population.rows.length < population.count ? ` · showing ${population.rows.length}` : ''}</p><ul>${rows}</ul></main>`;
};
