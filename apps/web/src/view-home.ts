...
import { asciiBar, codeClass, cutoffProgress, escapeHtml, formatAge, formatEastern, formatTapeStamp, formatTminus, healthBar } from './html';
...
const funnelItems = (snapshot: CockpitSnapshot) => {
	...
	const rows = stages
		.map(([stage, label, count]) => `<li><a href="/funnel/${stage}">${escapeHtml(label)}</a><span class="lead"></span><span>${count}</span></li>`)
		.join('');
...
const footer = (snapshot: CockpitSnapshot) => {
	...
	return `<footer class="chrome footer"><span>paper · drag a pane corner · dblclick title to reset</span><span>${escapeHtml(label)} <span class="bar">${escapeHtml(bar)}</span></span></footer>`;
};
